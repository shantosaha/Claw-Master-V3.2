                    ...
                    <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
                        ...
                        <div className="bg-muted/50 rounded-lg p-3">
                            <p className="text-xs text-muted-foreground mb-1">
                                Assigned Status
                                {assignmentCount > 1 && (
                                    <span className="text-amber-600 ml-1">({assignmentCount} machines)</span>
                                )}
                            </p>
                            {assignmentCount > 1 ? (
                                // Multi-machine: Read-only display with tooltip
                                <div
                                    className={`h-auto py-1 px-2 ${currentAssignedStatus.color} border rounded-md text-sm font-medium cursor-default`}
                                    title="Use the Machine Assignments panel in sidebar to manage individual machine assignments"
                                >
                                    {computedAssignedStatus}
                                    <span className="text-xs opacity-70 ml-1">(Use sidebar to manage)</span>
                                </div>
                            ) : (
